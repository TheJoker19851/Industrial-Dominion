...
begin
  ...
  v_price_per_unit := v_output_base_price;
  v_gross_amount := v_output_base_price * p_output_amount;
  ...
  values (
    ...
    jsonb_build_object(
      'orderId', v_order_id,
      'pricePerUnit', v_output_base_price,
  ...
  order_id := v_order_id;
  price_per_unit := v_output_base_price;
...
revoke all on function public.execute_decision_process_and_sell_local(uuid, text, bigint, text, text, bigint, bigint, text, numeric) from public;
